...
|----|------|----------|----------|------|------|
| REQ-UM-001 | 이메일 회원가입 | 이메일/비밀번호 기반 회원가입 기능 | 상 | ❌ | Google OAuth로 대체 |
| REQ-UM-002 | 소셜 로그인 | Google, Kakao 등 소셜 계정 연동 회원가입 | 중 | ✅ | Google OAuth2만 구현 |
| REQ-UM-003 | 이메일 인증 | 회원가입 시 이메일 인증 절차 | 상 | ❌ | Google OAuth가 대체 |
...
| REQ-FA-003 | 주기적 크롤링 | API 외 공고 데이터 주기적 크롤링 파이프라인 | 중 | ❌ | 수동 스크립트만 |
| REQ-FA-101 | 공고 벡터화 | 수집된 공고문을 임베딩하여 벡터DB에 저장 | 상 | ✅ | startup_funding_db |
| REQ-FA-102 | 자동 갱신 | 공고 변경/추가 시 벡터DB 자동 업데이트 | 상 | ❌ | 수동 빌드만 |
| REQ-FA-201 | 기업조건 매칭 | 업력, 매출, 업종 등 조건에 맞는 공고 필터링 | 상 | 🔧 | 벡터 검색 기반, 조건 필터 미구현 |
| REQ-FA-202 | 마감일 기반 정렬 | 마감일 임박 순으로 공고 우선순위 정렬 | 상 | ❌ | |
| REQ-FA-203 | 최신순 기반 정렬 | 최근 올라온 공고를 기준으로 정렬 | 상 | ❌ | |
| REQ-FA-204 | Top N 추천 | 조건에 맞는 상위 N개 공고 추천 기능 | 상 | 🔧 | 벡터 유사도 기반 검색 |
...
| REQ-HR-003 | Hierarchical RAG | 법령-시행령-사내규정 계층화 검색 | 상 | ❌ | Flat 검색 구조 |
...
| ID | 기능 | 상세 설명 | 우선순위 | 상태 | 비고 |
|----|------|----------|----------|------|------|
| REQ-PA-201 | 웹 푸시 알림 | 브라우저 기반 웹 푸시 알림 발송 | 상 | ❌ | Service Worker 미구현 |
| REQ-PA-202 | 답변 완료 알림 | AI 답변 생성 완료 시 푸시 알림 | 상 | 🔧 | 프론트엔드 토스트만 |
...
| REQ-CP-003 | 사업자등록증 업로드 | 사업자등록증 이미지/PDF 업로드 및 OCR 추출 | 중 | 🔧 | 업로드만, OCR 미구현 |
...
| ID | 기능 | 상세 설명 | 우선순위 | 상태 | 비고 |
|----|------|----------|----------|------|------|
| REQ-SC-001 | 비밀번호 암호화 | 비밀번호 해시 암호화 저장 (bcrypt) | 상 | ❌ | Google OAuth 사용 (해당 없음) |
| REQ-SC-002 | 민감정보 암호화 | 기업 민감정보 AES-256 암호화 저장 | 상 | ❌ | |
...
| REQ-SC-012 | 세션 관리 | 세션 타임아웃 및 동시 로그인 제어 | 중 | 🔧 | 타임아웃만, 동시 로그인 미제어 |
...
### 미구현 주요 항목
- **알림 시스템**: 마감일 알림, 신규 공고 알림, 웹 푸시 (REQ-PA-*)
- **문서 생성 확장**: 취업규칙, 연차관리, 급여명세서, HWP 형식 (REQ-AE-003~005, 102)
- **지원사업 고급 기능**: 자동 갱신, 조건 필터, 마감일/최신순 정렬 (REQ-FA-102~203)
- **기업 자격 분석**: OCR, 자동 자격 분석, 매칭 스코어 (REQ-CP-101~102)
- **회원 관리**: 관리자 회원 CRUD (REQ-AD-001~004)
