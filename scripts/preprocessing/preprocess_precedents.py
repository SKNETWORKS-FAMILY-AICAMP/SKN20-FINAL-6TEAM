"""
법령해석례(판례) JSON 전처리 스크립트
- 대상 파일: 02_smba_expc_full.json ~ 05_ftc_expc_full.json
- 출력 형식: JSONL (RAG용)
"""
...
def extract_metadata(item: dict, org: str, org_type: str) -> dict:
    """메타데이터 추출"""
    return {
        "source": "precedent",
        ...
        "org_type": org_type,  # 원본 파일의 기관 유형
    }
...
def process_precedent_file(input_path: Path, output_path: Path) -> dict:
    ...
    for item in items:
        ...
        chunk = {
            ...
            "metadata": extract_metadata(item, org, org_type)
        }
...
def main():
    ...
    # 처리 대상 파일들 (02~05)
    target_files = [
        ("02_smba_expc_full.json", "02_smba_expc_chunks.jsonl"),
        ("03_labor_expc_full.json", "03_labor_expc_chunks.jsonl"),
        ("04_nts_expc_full.json", "04_nts_expc_chunks.jsonl"),
        ("05_ftc_expc_full.json", "05_ftc_expc_chunks.jsonl"),
    ]

    print("=" * 60)
    print("법령해석례(판례) 전처리 시작")
    print("=" * 60)
    ...
    for input_name, output_name in target_files:
        input_path = law_dir / input_name
        output_path = law_dir / output_name
        ...
        total_stats["total_chunks"] += stats["processed"]

    print("\n" + "=" * 60)
    print("전처리 완료")
    print("=" * 60)
    print(f"처리된 파일: {total_stats['files_processed']}개")
    print(f"총 항목 수: {total_stats['total_items']}개")
    print(f"생성된 청크: {total_stats['total_chunks']}개")

    # 출력 파일 확인
    print("\n생성된 파일:")
    for _, output_name in target_files:
        output_path = law_dir / output_name
